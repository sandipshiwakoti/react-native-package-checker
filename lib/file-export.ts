    ...
    const packageData = Object.entries(packages).map(([name, info]) => {
        ...
        return {
            ...
            status: {
                ...
                maintenance: info.notInDirectory ? 'N/A' : info.unmaintained ? 'Unmaintained' : 'Maintained',
            },
            links: {
...
export const generatePDF = (data: FileExportData, options: PDFExportOptions = {}) => {
    ...
    const summaryData = [
        ...
        [
            { content: '' },
            { content: `Untested: ${data.summary.untested}` },
            { content: '' },
        ],
    ];
    ...
    autoTable(doc, {
        head: [[
            '#', 'Package', 'New Architecture', 'Maintenance', 'Score', 'Stars',
            'Last Updated', 'Platforms', 'Links', 'Alternatives'
        ]],
        body: data.packages
            .sort((a, b) => {
            ...
            .map((pkg, index) => [
                (index + 1).toString(),
                pkg.name,
                pkg.notInDirectory ? 'N/A' : pkg.status.newArchitecture.charAt(0).toUpperCase() + pkg.status.newArchitecture.slice(1).toLowerCase(),
                ...
                {
                    content: pkg.links.github && pkg.links.npm ?
                        `GitHub: ${pkg.links.github.replace(/(?:https?:\/\/)?(?:www\.)?/i, '')}\nNPM: ${pkg.links.npm.replace(/(?:https?:\/\/)?(?:www\.)?/i, '')}` :
                        pkg.links.github ? `GitHub: ${pkg.links.github.replace(/(?:https?:\/\/)?(?:www\.)?/i, '')}` :
                            pkg.links.npm ? `NPM: ${pkg.links.npm.replace(/(?:https?:\/\/)?(?:www\.)?/i, '')}` : 'N/A',
                    styles: {
                        cellWidth: 45,
                        overflow: 'linebreak',
                        fontStyle: 'normal'
